...
class tdo_rup(gr.top_block):
    ...
    def __init__(
        ...
        self.blocks_wavfile_sink_0 = blocks.wavfile_sink(
            wav_file_path, 1, samp_rate, blocks.FORMAT_WAV, blocks.FORMAT_PCM_16, False
        )
        self.blocks_multiply_xx_0_0 = blocks.multiply_vcc(1)
        self.blocks_multiply_xx_0 = blocks.multiply_vcc(1)
        self.blocks_multiply_const_vxx_0 = blocks.multiply_const_cc(mult_amp)
        self.blocks_message_strobe_0 = blocks.message_strobe(
            pmt.intern(f"{message_str}"), 500
        )
        ...
        self.analog_sig_source_x_0_0 = analog.sig_source_c(
            samp_rate, analog.GR_COS_WAVE, tx_rx_mix_freq, 1, 0, 0
        )
        ...
        ##################################################
        self.msg_connect(
            (self.blocks_message_strobe_0, "strobe"), (self.lora_tx_0, "in")
        )
        self.connect((self.analog_sig_source_x_0, 0), (self.blocks_multiply_xx_0, 1))
        self.connect(
            (self.analog_sig_source_x_0_0, 0), (self.blocks_multiply_xx_0_0, 0)
        )
        self.connect((self.audio_source_0, 0), (self.blocks_float_to_complex_0, 0))
        self.connect((self.blocks_complex_to_float_0, 0), (self.audio_sink_0, 0))
        self.connect(
            (self.blocks_complex_to_float_1, 0), (self.blocks_wavfile_sink_0, 0)
        )
        self.connect(
            (self.blocks_conjugate_cc_0, 0), (self.blocks_complex_to_float_0, 0)
        )
        self.connect(
            (self.blocks_float_to_complex_0, 0), (self.blocks_multiply_xx_0, 0)
        )
        self.connect(
            (self.blocks_multiply_const_vxx_0, 0), (self.blocks_conjugate_cc_0, 0)
        )
        self.connect(
            (self.blocks_multiply_xx_0, 0), (self.blocks_complex_to_float_1, 0)
        )
        self.connect((self.blocks_multiply_xx_0, 0), (self.lora_rx_0, 0))
        self.connect(
            (self.blocks_multiply_xx_0_0, 0), (self.blocks_multiply_const_vxx_0, 0)
        )
...
def argument_parser():
    ...
    parser.add_argument(
        ...
        dest="mult_amp",
        type=eng_float,
        default=eng_notation.num_to_str(float(0.5)),
...
def main(top_block_cls=tdo_rup, options=None):
    ...
    tb = top_block_cls(
        ...
        mult_amp=options.mult_amp,
        tx_cr=options.tx_cr,
        tx_rx_bw=options.tx_rx_bw,
        tx_rx_mix_freq=options.tx_rx_mix_freq,
        tx_rx_sf=options.tx_rx_sf,
        wav_file_path=options.wav_file_path,
    )
