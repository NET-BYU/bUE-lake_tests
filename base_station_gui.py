...
class TestDialog:
    ...
    def __init__(self, parent, base_station):
        ...
        self.test_files = ["lora_td_ru", "lora_tu_rd", "Old/helloworld", "gpstest", "gpstest2"]
    ...
    def setup_dialog(self):
        ...
        for i, bue_id in enumerate(self.base_station.connected_bues):
            ...
            cb = ttk.Checkbutton(checkbox_frame, text=bue_name, variable=var, 
        ...
        self.no_selection_label = ttk.Label(self.scrollable_frame, 
        ...
        self.run_btn = ttk.Button(button_frame, text="Run Tests", 
                                command=self.run_tests, state=tk.DISABLED)
    ...
    def show_inline_configs(self):
        ...
        for i, bue_id in enumerate(self.selected_bues):
            ...
            file_combo = ttk.Combobox(file_frame, textvariable=file_var, 
                                    values=self.test_files, state="readonly", width=15)
    ...
    def run_tests(self):
        ...
        if not self.selected_bues or not hasattr(self, 'config_widgets'):
            messagebox.showwarning("No Configuration", "Please select and configure at least one bUE for testing")
            return
        ...
        try:
            ...
            messagebox.showinfo("Tests Scheduled", 
...
class ConfigureBueDialog:
    ...
    def setup_dialog(self):
        ...
        ttk.Label(self.dialog, text=f"Configure test for {bUEs.get(str(self.bue_id), f'bUE {self.bue_id}')}").pack(pady=10)
        
        # Test file selection
        ttk.Label(self.dialog, text="Test File:").pack(anchor=tk.W, padx=20)
        self.file_var = tk.StringVar(value=self.test_files[0])
        file_combo = ttk.Combobox(self.dialog, textvariable=self.file_var, values=self.test_files, state="readonly")
    ...
    def save_config(self):
        ...
        self.config_tree.insert('', 'end', text=bue_name, 
...
class AddMarkerDialog:
    ...
    def setup_dialog(self):
        ...
        try:
            bue_options = ["None"] + [bUEs.get(str(bue_id), f"bUE {bue_id}") for bue_id in self.main_gui.base_station.connected_bues]
            self.bue_var = tk.StringVar(value="None")
            ttk.Combobox(self.dialog, textvariable=self.bue_var, values=bue_options, state="readonly").pack(fill=tk.X, padx=20, pady=5)
        except Exception as e:
            print(f"Error creating bUE combobox: {e}")
            # Fallback simple combobox
            self.bue_var = tk.StringVar(value="None")
            ttk.Combobox(self.dialog, textvariable=self.bue_var, values=["None"], state="readonly").pack(fill=tk.X, padx=20, pady=5)
...
class ManageMarkersDialog:
    ...
    def refresh_markers(self):
        ...
        for marker_id, marker in self.main_gui.custom_markers.items():
            ...
            self.markers_tree.insert('', 'end', iid=marker_id, text=marker['name'],
...
class LogViewerDialog:
    ...
    def clear_log(self):
        ...
        if messagebox.askyesno("Clear Display", "Clear the log display? (This won't delete the actual log file)"):
            self.log_text.config(state=tk.NORMAL)
            self.log_text.delete(1.0, tk.END)
            self.log_text.config(state=tk.DISABLED)
