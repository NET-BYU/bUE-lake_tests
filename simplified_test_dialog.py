...
class TestDialog:
    ...
    def __init__(self, parent, base_station):
        ...
        self.test_files = ["lora_td_ru", "lora_tu_rd", "helloworld", "gpstest", "gpstest2"]
    ...
    def setup_dialog(self):
        ...
        ttk.Label(config_frame, text="Click 'Configure bUEs' after selecting which ones to test").pack(anchor=tk.W)
        ...
        self.config_btn = ttk.Button(button_frame, text="Configure bUEs", 
                                   command=self.configure_selected_bues, state=tk.DISABLED)
    ...
    def run_tests(self):
        ...
        try:
            ...
            today = date.today()
            start_time = datetime.combine(today, datetime.min.time().replace(hour=hour, minute=minute, second=second))
...
class IndividualBueConfigDialog:
    ...
    def setup_dialog(self):
        ...
        ttk.Label(header_frame, text=f"Configure test for {bue_name}", 
        ...
        for i, test_file in enumerate(self.test_files):
            ttk.Radiobutton(file_frame, text=test_file, variable=self.file_var, 
                          value=test_file).pack(anchor=tk.W, padx=20)
